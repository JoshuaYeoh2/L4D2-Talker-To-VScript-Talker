...
def convert_criterion_line(line):
    ...
    # Standard rule
    match = re.match(r'criterion\s+"([^"]+)"\s+("?)([A-Za-z0-9_]+)\2\s+"([^"]+?)"', line, re.IGNORECASE)
    if not match:
        return None

    func_name, _, field, value = match.groups()
    ...
    if re.match(r'^-?\d+(\.\d+)?$', val):
        field_expr = f'query.{field.lower()}'
    else:
        # val = f'"{val.lower()}"'
        val = f'"{val}"'
        # field_expr = f'query.{field.lower()}.tostring().tolower()'
        field_expr = f'query.{field.lower()}'

    safe_func_name = sanitize_func_name(func_name)
    return f"function {safe_func_name}(query) {{ return {field_expr} {op} {val}; }}"
...
# convert_all("talker", "1_criterias.nut", just_one_file="finales_dlc2.txt")
